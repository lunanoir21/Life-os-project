...
pub async fn get_weekly_review(
    ...
    let tasks_created: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM Task WHERE createdAt >= ? AND createdAt <= ?")
            .bind(week_start)
            .bind(week_end)
            .fetch_one(&st.db)
            .await?;
    ...
    let all_done: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM Task WHERE status = 'done' AND archived = 0")
            .fetch_one(&st.db)
            .await?;
    let task_completion_rate = if all_active > 0 {
        (all_done * 100) / all_active
    } else {
        0
    };
    ...
    let top_completed: Vec<serde_json::Value> = sorted_tasks
        .iter()
        .take(5)
        .map(|r| {
            serde_json::json!({
    ...
    let habit_completion_rate = if total_possible > 0 {
        (total_habit_logs * 100) / total_possible
    } else {
        0
    };
    ...
    let total_focus_time: i64 = time_entries
        .iter()
        .map(|r| r.try_get::<i64, _>("duration").unwrap_or(0))
        .sum();
    ...
    let moods: Vec<i64> = journal_entries
        .iter()
        .filter_map(|r| r.try_get::<Option<i64>, _>("moodScore").ok().flatten())
        .collect();
    let energies: Vec<i64> = journal_entries
        .iter()
        .filter_map(|r| r.try_get::<Option<i64>, _>("energy").ok().flatten())
        .collect();
    ...
    let avg_energy = if !energies.is_empty() {
        energies.iter().sum::<i64>() as f64 / energies.len() as f64
    } else {
        0.0
    };
    ...
    let income: f64 = transactions
        ...
        .map(|r| crate::utils::row_f64(r, "amount"))
        .sum();
    ...
    for r in transactions.iter().filter(|r| {
        r.try_get::<String, _>("type")
            .map(|s| s == "expense")
            .unwrap_or(false)
    }) {
    ...
    if !tasks_completed.is_empty() {
        highlights.push(format!(
            ...
            tasks_completed.len(),
            if tasks_completed.len() > 1 { "s" } else { "" },
            if tasks_completed.len() >= 5 {
                " — great productivity!"
            } else {
                ""
            }
    ...
    if habit_completion_rate >= 80 {
        highlights.push(format!(
            "Habit completion rate of {}% — outstanding consistency!",
            habit_completion_rate
        ));
    } else if habit_completion_rate >= 50 {
        highlights.push(format!(
            "Habit completion at {}% — keep building that momentum!",
            habit_completion_rate
        ));
    }
    if max_streak >= 7 {
        highlights.push(format!(
            "\"{}\" streak reached {} days 🔥",
            max_streak_name, max_streak
        ));
    }
    if total_focus_time > 0 {
        highlights.push(format!(
            "Logged {:.1} hours of focused work",
            total_focus_time as f64 / 60.0
        ));
    }
    if pomodoro_sessions > 0 {
        highlights.push(format!(
            ...
            pomodoro_sessions,
            if pomodoro_sessions > 1 { "s" } else { "" }
        ));
    }
    if income > expenses {
        let net = income - expenses;
        highlights.push(format!("Net savings of ${:.2} this week", net));
    }
    if mood_trend == "improving" {
        highlights.push("Mood trending upward this week 😊".to_string());
    }
    if highlights.len() < 3 {
        if tasks_completed.is_empty() {
            highlights.push("No tasks completed this week — set a goal for next week!".to_string());
        }
        if highlights.len() < 3 {
            highlights
                .push("A new week is ahead — plan your priorities and set intentions".to_string());
        }
    }
    ...
    let finance_s = if income > expenses {
        ((income - expenses) / income.max(1.0) * 100.0).min(100.0) * 0.1
    } else {
        0.0
    };
    let week_score = (task_s + habit_s + focus_s + mood_s + energy_s + finance_s).min(100.0) as i64;

    let week_start_str = crate::prisma_dt::PrismaDateTime(week_start)
        .to_iso()
        .split('T')
        .next()
        .unwrap_or("")
        .to_string();
    let week_end_str = crate::prisma_dt::PrismaDateTime(week_end)
        .to_iso()
        .split('T')
        .next()
        .unwrap_or("")
