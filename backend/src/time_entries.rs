...
pub(crate) struct TimeEntry {
    ...
    start_time: PrismaDateTime, end_time: Option<PrismaDateTime>,
    duration: Option<i64>, billable: bool,
    task_id: Option<String>,
    created_at: PrismaDateTime, updated_at: PrismaDateTime,
    task: Option<TaskMini>,
}

fn entry_from_row(r: &sqlx::sqlite::SqliteRow, task: Option<TaskMini>) -> Result<TimeEntry, sqlx::Error> {
    Ok(TimeEntry {
        id: r.try_get("id")?,
...
pub async fn create_time_entry(
    ...
    let row = sqlx::query("SELECT * FROM TimeEntry WHERE id = ?").bind(&id).fetch_one(&st.db).await?;
...
pub async fn stop_time_entry(
    ...
    let id = body.get("id").and_then(|v| v.as_str()).filter(|s| !s.is_empty())
        .ok_or_else(|| AppError::BadRequest("id is required".to_string()))?.to_string();
...
pub async fn delete_time_entry(
    ...
    if exists == 0 { return Err(AppError::NotFound("Time entry not found".to_string())); }
    sqlx::query("DELETE FROM TimeEntry WHERE id = ?").bind(&id).execute(&st.db).await?;
