...
pub(crate) struct TaskStatus { by_status: serde_json::Map<String, serde_json::Value>, total: i64 }
...
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct HabitLogMini { date: PrismaDateTime, count: i64 }

#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct HabitsInfo {
    total: i64, completed_today: i64, completion_rate: i64,
...
pub(crate) struct FinanceInfo {
    accounts: Vec<AccountMini>, total_balance: f64,
    monthly_income: f64, monthly_expenses: f64,
}

#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct RecentNote {
    id: String, title: String, r#type: String,
    icon: Option<String>, color: Option<String>,
    updated_at: PrismaDateTime,
    folder: Option<FolderMini>,
}
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct FolderMini { id: String, name: String }

#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct UpcomingEvent {
    id: String, title: String, start_date: PrismaDateTime,
    end_date: Option<PrismaDateTime>, all_day: bool, color: String,
    task: Option<EventTaskMini>,
}
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct EventTaskMini { id: String, title: String }

#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct JournalMini { id: String, title: Option<String>, mood: Option<String>, mood_score: Option<i64>, date: PrismaDateTime }

#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct GoalMini { id: String, title: String, progress: i64, category: String, target_date: Option<PrismaDateTime> }

#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct ProjectCount { tasks: i64 }
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
...
pub async fn get_dashboard(State(st): State<AppState>) -> Result<Json<Value>, AppError> {
    ...
    let status_rows = sqlx::query(
        "SELECT status, COUNT(*) as cnt FROM Task WHERE archived = 0 GROUP BY status"
    ...
    let tasks = TaskStatus { by_status, total: task_total };
    ...
    let completed_today: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM HabitLog WHERE date >= ? AND date <= ?"
    ).bind(today_ms).bind(today_end).fetch_one(&st.db).await?;
    let habit_rate = if total_habits > 0 { (completed_today * 100) / total_habits } else { 0 };
    ...
    for hr in &habit_rows {
        ...
        let log_rows = sqlx::query("SELECT date, count FROM HabitLog WHERE habitId = ? ORDER BY date DESC LIMIT 7")
            .bind(&hid).fetch_all(&st.db).await?;
        let logs = log_rows.iter().map(|l| Ok(HabitLogMini {
            date: PrismaDateTime(l.try_get::<i64, _>("date")?),
            count: l.try_get("count")?,
        })).collect::<Result<Vec<_>, sqlx::Error>>()?;
        recent_activity.push(HabitActivity {
            id: hid, name: hr.try_get("name")?, icon: hr.try_get("icon")?,
            color: hr.try_get("color")?, logs,
        });
    }
    let habits = HabitsInfo { total: total_habits, completed_today, completion_rate: habit_rate, recent_activity };
    ...
    for ar in &acc_rows {
        ...
        accounts.push(AccountMini {
            id: ar.try_get("id")?, name: ar.try_get("name")?, r#type: ar.try_get("type")?,
            balance: bal, currency: ar.try_get("currency")?,
            color: ar.try_get("color")?, icon: ar.try_get("icon")?,
        });
    }
    let month_start_ms = {
        use chrono::{Datelike, TimeZone, Utc};
        let dt = Utc.timestamp_millis_opt(now).unwrap();
        Utc.with_ymd_and_hms(dt.year(), dt.month(), 1, 0, 0, 0).unwrap().timestamp_millis()
    };
    ...
    let finance = FinanceInfo { accounts, total_balance, monthly_income, monthly_expenses };
    ...
    for nr in &note_rows {
        ...
        let folder = if let Some(ref fid) = folder_id {
            let fr = sqlx::query("SELECT id, name FROM NoteFolder WHERE id = ?")
                .bind(fid).fetch_optional(&st.db).await?;
            fr.map(|r| FolderMini {
        ...
        recent_notes.push(RecentNote {
            id: nr.try_get("id")?, title: nr.try_get("title")?,
            r#type: nr.try_get("type")?, icon: nr.try_get("icon")?,
    ...
    for er in &event_rows {
        ...
        let task = if let Some(ref tid) = task_id {
            let tr = sqlx::query("SELECT id, title FROM Task WHERE id = ?")
                .bind(tid).fetch_optional(&st.db).await?;
        ...
        upcoming_events.push(UpcomingEvent {
            id: er.try_get("id")?, title: er.try_get("title")?,
    ...
    let recent_journal_entries = journal_rows.iter().map(|r| Ok(JournalMini {
        id: r.try_get("id")?, title: r.try_get("title")?,
        mood: r.try_get("mood")?, mood_score: r.try_get("moodScore")?,
    ...
    let active_goals = goal_rows.iter().map(|r| Ok(GoalMini {
        id: r.try_get("id")?, title: r.try_get("title")?,
        progress: r.try_get("progress")?, category: r.try_get("category")?,
    ...
    for pr in &project_rows {
        ...
        let tc: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM Task WHERE projectId = ?")
            .bind(&pid).fetch_one(&st.db).await?;
        active_projects.push(ProjectMini {
            id: pid, name: pr.try_get("name")?, color: pr.try_get("color")?,
