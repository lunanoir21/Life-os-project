...
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct HabitTagFull { id: String, habit_id: String, tag_id: String, tag: TagMini }
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct HabitLog {
    id: String, habit_id: String, date: PrismaDateTime, count: i64,
    note: Option<String>, created_at: PrismaDateTime,
}
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct HabitLogCount { logs: i64 }
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct Habit {
    id: String, name: String, description: Option<String>, icon: Option<String>,
    color: String, frequency: String, frequency_config: Option<String>,
    target_count: i64, unit: Option<String>, reminder_enabled: bool,
    reminder_time: Option<String>, archived: bool,
    created_at: PrismaDateTime, updated_at: PrismaDateTime,
}
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct HabitFull {
    #[serde(flatten)] habit: Habit,
...
fn habit_from_row(r: &sqlx::sqlite::SqliteRow) -> Result<Habit, sqlx::Error> {
    Ok(Habit {
        ...
        icon: r.try_get("icon")?, color: r.try_get("color")?, frequency: r.try_get("frequency")?,
        frequency_config: r.try_get("frequencyConfig")?, target_count: r.try_get("targetCount")?,
        unit: r.try_get("unit")?, reminder_enabled: r.try_get::<i64,_>("reminderEnabled")? != 0,
        reminder_time: r.try_get("reminderTime")?, archived: r.try_get::<i64,_>("archived")? != 0,
...
async fn habit_tags(db: &SqlitePool, habit_id: &str) -> Result<Vec<HabitTagFull>, sqlx::Error> {
    ...
    ).bind(habit_id).fetch_all(db).await?;
    rows.iter().map(|r| Ok(HabitTagFull {
        id: r.try_get("id")?, habit_id: r.try_get("habitId")?, tag_id: r.try_get("tagId")?,
        tag: TagMini { id: r.try_get("tid")?, name: r.try_get("tname")?, color: r.try_get("tcolor")? },
    })).collect()
}

async fn habit_logs(db: &SqlitePool, habit_id: &str, take: Option<i64>) -> Result<Vec<HabitLog>, sqlx::Error> {
    let sql = match take {
...
async fn build_habit_full(db: &SqlitePool, habit: Habit, log_limit: Option<i64>) -> Result<HabitFull, sqlx::Error> {
    let tags = habit_tags(db, &habit.id).await?;
    let log_count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM HabitLog WHERE habitId = ?")
        .bind(&habit.id).fetch_one(db).await?;
    let logs = habit_logs(db, &habit.id, log_limit).await?;
    Ok(HabitFull { habit, tags, logs, count: HabitLogCount { logs: log_count } })
}
...
pub async fn create_habit(
    ...
    .execute(&st.db).await?;
    let row = sqlx::query("SELECT * FROM Habit WHERE id = ?").bind(&id).fetch_one(&st.db).await?;
    let h = habit_from_row(&row)?;
    Ok((StatusCode::CREATED, Json(build_habit_full(&st.db, h, None).await?)))
}
...
pub async fn get_habit(
    ...
    Ok(Json(build_habit_full(&st.db, habit_from_row(&row)?, None).await?))
}
...
pub async fn update_habit(
    ...
    if exists == 0 { return Err(AppError::NotFound("Habit not found".to_string())); }
    let now = now_ms();
    ...
    if let Some(v) = body.get("frequency").and_then(|v| v.as_str()) { crate::push_set!(qb, first, "frequency = ", v.to_string()); }
    if let Some(v) = patch_str(&body, "frequencyConfig") { crate::push_set!(qb, first, "frequencyConfig = ", v); }
    if let Some(v) = body.get("targetCount").and_then(|v| v.as_i64()) { crate::push_set!(qb, first, "targetCount = ", v); }
    if let Some(v) = patch_str(&body, "unit") { crate::push_set!(qb, first, "unit = ", v); }
    if let Some(v) = patch_bool(&body, "reminderEnabled") { crate::push_set!(qb, first, "reminderEnabled = ", if v {1_i64} else {0_i64}); }
    if let Some(v) = patch_str(&body, "reminderTime") { crate::push_set!(qb, first, "reminderTime = ", v); }
    ...
    let row = sqlx::query("SELECT * FROM Habit WHERE id = ?").bind(&id).fetch_one(&st.db).await?;
    Ok(Json(build_habit_full(&st.db, habit_from_row(&row)?, Some(30)).await?))
}
