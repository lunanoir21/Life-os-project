...
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct GoalTagFull { id: String, goal_id: String, tag_id: String, tag: TagMini }
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct Milestone {
    id: String, goal_id: String, title: String, completed: bool,
    completed_at: Option<PrismaDateTime>, order: i64,
    created_at: PrismaDateTime, updated_at: PrismaDateTime,
}
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct ProjectMini { id: String, name: String, color: String }
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct GoalProjectRef {
    id: String, goal_id: String, project_id: String, project: ProjectMini,
}
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct Goal {
    id: String, title: String, description: Option<String>, category: String,
    status: String, progress: i64, start_date: Option<PrismaDateTime>,
    target_date: Option<PrismaDateTime>, completed_at: Option<PrismaDateTime>,
    archived: bool, parent_goal_id: Option<String>,
    created_at: PrismaDateTime, updated_at: PrismaDateTime,
}
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
...
fn goal_from_row(r: &sqlx::sqlite::SqliteRow) -> Result<Goal, sqlx::Error> {
    Ok(Goal {
        id: r.try_get("id")?, title: r.try_get("title")?, description: r.try_get("description")?,
        category: r.try_get("category")?, status: r.try_get("status")?, progress: r.try_get("progress")?,
        start_date: r.try_get::<Option<i64>,_>("startDate")?.map(PrismaDateTime),
        target_date: r.try_get::<Option<i64>,_>("targetDate")?.map(PrismaDateTime),
...
async fn goal_tags(db: &SqlitePool, goal_id: &str) -> Result<Vec<GoalTagFull>, sqlx::Error> {
    ...
    rows.iter().map(|r| Ok(GoalTagFull {
        id: r.try_get("id")?, goal_id: r.try_get("goalId")?, tag_id: r.try_get("tagId")?,
        tag: TagMini { id: r.try_get("tid")?, name: r.try_get("tname")?, color: r.try_get("tcolor")? },
    })).collect()
}

async fn milestones_for_goal(db: &SqlitePool, goal_id: &str) -> Result<Vec<Milestone>, sqlx::Error> {
    let rows = sqlx::query("SELECT * FROM Milestone WHERE goalId = ? ORDER BY \"order\" ASC")
        .bind(goal_id).fetch_all(db).await?;
    rows.iter().map(|r| Ok(Milestone {
        id: r.try_get("id")?, goal_id: r.try_get("goalId")?, title: r.try_get("title")?,
...
async fn projects_for_goal(db: &SqlitePool, goal_id: &str) -> Result<Vec<GoalProjectRef>, sqlx::Error> {
    let rows = sqlx::query(
    ...
    rows.iter().map(|r| Ok(GoalProjectRef {
        id: r.try_get("id")?, goal_id: r.try_get("goalId")?, project_id: r.try_get("projectId")?,
        project: ProjectMini { id: r.try_get("pid")?, name: r.try_get("pname")?, color: r.try_get("pcolor")? },
    })).collect()
}
...
async fn subgoals_for_goal(db: &SqlitePool, goal_id: &str) -> Result<Vec<SubGoal>, sqlx::Error> {
    ...
    for r in &rows {
        ...
        out.push(SubGoal {
            ...
            status: g.status, progress: g.progress, start_date: g.start_date,
            target_date: g.target_date, completed_at: g.completed_at, archived: g.archived,
            parent_goal_id: g.parent_goal_id, created_at: g.created_at, updated_at: g.updated_at,
            milestones,
        });
    }
    Ok(out)
...
pub async fn list_goals(
    ...
    if let Some(c) = params.get("category") { qb.push(" AND category = ").push_bind(c.clone()); }
    ...
    for r in &rows {
        ...
        out.push(GoalFull { goal, tags, milestones, subgoals, projects });
    }
    Ok(Json(out))
}
...
pub async fn create_goal(
    ...
    if let Some(ms) = body.get("milestones").and_then(|v| v.as_array()) {
        for m in ms {
            let mid = gen_id();
            let mtitle = m.get("title").and_then(|v| v.as_str()).unwrap_or("").to_string();
    ...
    let row = sqlx::query("SELECT * FROM Goal WHERE id = ?").bind(&id).fetch_one(&st.db).await?;
    ...
    Ok((StatusCode::CREATED, Json(GoalFull { goal, tags, milestones, subgoals, projects })))
}
...
pub async fn get_goal(
    ...
    let parent_goal = if let Some(ref pid) = goal.parent_goal_id {
        sqlx::query("SELECT * FROM Goal WHERE id = ?").bind(pid).fetch_optional(&st.db).await?
            .map(|r| goal_from_row(&r)).transpose()?.map(Box::new)
    } else { None };
    let tags = goal_tags(&st.db, &id).await?;
    let milestones = milestones_for_goal(&st.db, &id).await?;
    let subgoals = subgoals_for_goal(&st.db, &id).await?;
    let projects = projects_for_goal(&st.db, &id).await?;
    Ok(Json(GoalDetail { goal, tags, milestones, subgoals, projects, parent_goal }))
}
...
pub async fn update_goal(
    ...
    if exists == 0 { return Err(AppError::NotFound("Goal not found".to_string())); }
    let now = now_ms();
    ...
    if let Some(v) = body.get("category").and_then(|v| v.as_str()) { crate::push_set!(qb, first, "category = ", v.to_string()); }
    if let Some(v) = body.get("status").and_then(|v| v.as_str()) {
        crate::push_set!(qb, first, "status = ", v.to_string());
        if v == "completed" { crate::push_set!(qb, first, "completedAt = ", now); }
    }
    ...
    if let Some(v) = patch_ms(&body, "targetDate") { crate::push_set!(qb, first, "targetDate = ", v); }
    ...
    if let Some(ms) = body.get("milestones").and_then(|v| v.as_array()) {
        for m in ms {
            if let Some(mid) = m.get("id").and_then(|v| v.as_str()) {
                // Update existing
                let mtitle = m.get("title").and_then(|v| v.as_str()).unwrap_or("").to_string();
                let completed = m.get("completed").and_then(|v| v.as_bool()).unwrap_or(false);
                let completed_at: Option<i64> = if completed { Some(now) } else { None };
                sqlx::query("UPDATE Milestone SET title=?,completed=?,completedAt=?,updatedAt=? WHERE id=?")
                    .bind(&mtitle).bind(if completed {1_i64} else {0_i64}).bind(completed_at).bind(now).bind(mid)
                    .execute(&st.db).await?;
            } else {
                // Create new
                let new_id = gen_id();
    ...
    let subgoals = subgoals_for_goal(&st.db, &id).await?;
    let projects = projects_for_goal(&st.db, &id).await?;
    Ok(Json(GoalFull { goal, tags, milestones, subgoals, projects }))
}
...
pub async fn delete_goal(
    ...
    let exists: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM Goal WHERE id = ?")
        .bind(&id).fetch_one(&st.db).await?;
    if exists == 0 { return Err(AppError::NotFound("Goal not found".to_string())); }
    sqlx::query("DELETE FROM Goal WHERE id = ?").bind(&id).execute(&st.db).await?;
