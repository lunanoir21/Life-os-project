...
pub(crate) struct ProjectTaskCount { tasks: i64 }

#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct GoalRef { id: String, title: String }
...
pub(crate) struct ProjectTask {
    id: String, title: String, description: Option<String>, status: String, priority: String,
    due_date: Option<PrismaDateTime>, start_date: Option<PrismaDateTime>,
    completed_at: Option<PrismaDateTime>, estimated_minutes: Option<i64>,
    actual_minutes: Option<i64>, recurrence: Option<String>, recurrence_config: Option<String>,
    position: i64, archived: bool, created_at: PrismaDateTime, updated_at: PrismaDateTime,
    project_id: Option<String>, parent_task_id: Option<String>,
...
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct GoalRefFull { id: String, title: String, status: String }
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
...
async fn goals_for_project(db: &SqlitePool, pid: &str) -> Result<Vec<GoalProjectRef>, sqlx::Error> {
    ...
    rows.iter().map(|r| Ok(GoalProjectRef {
        id: r.try_get("id")?,
        goal_id: r.try_get("goalId")?,
        project_id: r.try_get("projectId")?,
        goal: GoalRef { id: r.try_get("gid")?, title: r.try_get("gtitle")? },
    })).collect()
}

async fn task_tags_for_task(db: &SqlitePool, task_id: &str) -> Result<Vec<TaskTagMini>, sqlx::Error> {
    let rows = sqlx::query(
    ...
    rows.iter().map(|r| Ok(TaskTagMini {
        id: r.try_get("id")?,
...
pub async fn list_projects(
    ...
    if let Some(a) = params.get("archived") {
        qb.push(" AND archived = ").push_bind(if a == "true" { 1_i64 } else { 0_i64 });
    }
    ...
    for r in &rows {
        ...
        let tasks: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM Task WHERE projectId = ?")
            .bind(&p.id).fetch_one(&st.db).await?;
        let goals = goals_for_project(&st.db, &p.id).await?;
        out.push(ProjectList { project: p, count: ProjectTaskCount { tasks }, goals });
    }
    Ok(Json(out))
}
...
pub async fn create_project(
    ...
    let row = sqlx::query("SELECT * FROM Project WHERE id = ?").bind(&id).fetch_one(&st.db).await?;
    let p = project_from_row(&row)?;
    Ok((StatusCode::CREATED, Json(ProjectList { project: p, count: ProjectTaskCount { tasks: 0 }, goals: vec![] })))
}
...
pub async fn get_project(
    ...
    for tr in &task_rows {
        ...
        let sub_rows = sqlx::query("SELECT * FROM Task WHERE parentTaskId = ?").bind(&tid).fetch_all(&st.db).await?;
        let subtasks = sub_rows.iter().map(|sr| Ok(SubTask {
            id: sr.try_get("id")?, title: sr.try_get("title")?, description: sr.try_get("description")?,
            status: sr.try_get("status")?, priority: sr.try_get("priority")?,
            due_date: sr.try_get::<Option<i64>,_>("dueDate")?.map(PrismaDateTime),
            start_date: sr.try_get::<Option<i64>,_>("startDate")?.map(PrismaDateTime),
            completed_at: sr.try_get::<Option<i64>,_>("completedAt")?.map(PrismaDateTime),
            estimated_minutes: sr.try_get("estimatedMinutes")?, actual_minutes: sr.try_get("actualMinutes")?,
            recurrence: sr.try_get("recurrence")?, recurrence_config: sr.try_get("recurrenceConfig")?,
            position: sr.try_get("position")?, archived: sr.try_get::<i64,_>("archived")? != 0,
            created_at: PrismaDateTime(sr.try_get::<i64,_>("createdAt")?),
            updated_at: PrismaDateTime(sr.try_get::<i64,_>("updatedAt")?),
            project_id: sr.try_get("projectId")?, parent_task_id: sr.try_get("parentTaskId")?,
        })).collect::<Result<Vec<_>, sqlx::Error>>()?;
        tasks.push(ProjectTask {
            id: tid, title: tr.try_get("title")?, description: tr.try_get("description")?,
            status: tr.try_get("status")?, priority: tr.try_get("priority")?,
            due_date: tr.try_get::<Option<i64>,_>("dueDate")?.map(PrismaDateTime),
            start_date: tr.try_get::<Option<i64>,_>("startDate")?.map(PrismaDateTime),
            completed_at: tr.try_get::<Option<i64>,_>("completedAt")?.map(PrismaDateTime),
            estimated_minutes: tr.try_get("estimatedMinutes")?, actual_minutes: tr.try_get("actualMinutes")?,
            recurrence: tr.try_get("recurrence")?, recurrence_config: tr.try_get("recurrenceConfig")?,
            position: tr.try_get("position")?, archived: tr.try_get::<i64,_>("archived")? != 0,
            created_at: PrismaDateTime(tr.try_get::<i64,_>("createdAt")?),
            updated_at: PrismaDateTime(tr.try_get::<i64,_>("updatedAt")?),
            project_id: tr.try_get("projectId")?, parent_task_id: tr.try_get("parentTaskId")?,
            tags, subtasks,
        });
    }
    let goals = goals_for_project(&st.db, &id).await?;
    Ok(Json(ProjectDetail { project: p, tasks, goals }))
}
...
pub async fn update_project(
    ...
    if exists == 0 { return Err(AppError::NotFound("Project not found".to_string())); }
    let now = now_ms();
    ...
    let row = sqlx::query("SELECT * FROM Project WHERE id = ?").bind(&id).fetch_one(&st.db).await?;
    let p = project_from_row(&row)?;
    let tasks: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM Task WHERE projectId = ?").bind(&id).fetch_one(&st.db).await?;
    let goals = goals_for_project(&st.db, &id).await?;
    Ok(Json(ProjectList { project: p, count: ProjectTaskCount { tasks }, goals }))
}
...
pub async fn delete_project(
    ...
    let exists: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM Project WHERE id = ?")
        .bind(&id).fetch_one(&st.db).await?;
    if exists == 0 { return Err(AppError::NotFound("Project not found".to_string())); }
    sqlx::query("DELETE FROM Project WHERE id = ?").bind(&id).execute(&st.db).await?;
