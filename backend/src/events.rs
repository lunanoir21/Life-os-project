...
async fn fetch_task_mini(
    db: &sqlx::SqlitePool,
    task_id: &str,
) -> Result<Option<TaskMini>, sqlx::Error> {
...
pub async fn list_events(
    ...
    let mut count_qb: QueryBuilder<Sqlite> =
        QueryBuilder::new("SELECT COUNT(*) FROM CalendarEvent WHERE 1=1");
    ...
    qb.push(" ORDER BY startDate ASC LIMIT ")
        .push_bind(limit)
        .push(" OFFSET ")
        .push_bind(offset);
...
pub async fn update_event(
    ...
    if let Some(v) = patch_bool(&body, "allDay") {
        crate::push_set!(qb, first, "allDay = ", if v { 1_i64 } else { 0_i64 });
    }
    if let Some(v) = patch_str(&body, "color") {
        crate::push_set!(qb, first, "color = ", v);
    }
    if let Some(v) = patch_str(&body, "location") {
        crate::push_set!(qb, first, "location = ", v);
    }
    if let Some(v) = patch_str(&body, "recurrence") {
        crate::push_set!(qb, first, "recurrence = ", v);
    }
    if let Some(v) = patch_str(&body, "recurrenceConfig") {
        crate::push_set!(qb, first, "recurrenceConfig = ", v);
    }
    if let Some(v) = patch_str(&body, "taskId") {
        crate::push_set!(qb, first, "taskId = ", v);
    }
...
pub async fn delete_event(
    ...
    sqlx::query("DELETE FROM CalendarEvent WHERE id = ?")
        .bind(&id)
        .execute(&st.db)
        .await?;
