...
pub(crate) struct TransactionCount { transactions: i64 }

#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct AccountBasic {
    id: String, name: String, r#type: String, balance: f64,
    currency: String, color: Option<String>, icon: Option<String>,
    is_default: bool, archived: bool,
...
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct CategoryMini { id: String, name: String, icon: Option<String>, color: String, r#type: String }

#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct TxnInAccount {
    id: String, amount: f64, description: String, r#type: String,
    date: PrismaDateTime, note: Option<String>, is_recurring: bool,
    recurrence: Option<String>, account_id: String, category_id: Option<String>,
    transfer_to_id: Option<String>, created_at: PrismaDateTime, updated_at: PrismaDateTime,
...
fn account_basic_from_row(r: &sqlx::sqlite::SqliteRow, count: i64) -> Result<AccountBasic, sqlx::Error> {
    Ok(AccountBasic {
        id: r.try_get("id")?, name: r.try_get("name")?, r#type: r.try_get("type")?,
        balance: crate::utils::row_f64(r, "balance"), currency: r.try_get("currency")?,
        color: r.try_get("color")?, icon: r.try_get("icon")?,
        ...
        count: TransactionCount { transactions: count },
    })
}

pub async fn list_accounts(State(st): State<AppState>) -> Result<Json<Vec<AccountBasic>>, AppError> {
...
pub async fn create_account(
    ...
    let name = body.get("name").and_then(|v| v.as_str()).map(str::trim).filter(|s| !s.is_empty())
        .ok_or_else(|| AppError::BadRequest("Name is required".to_string()))?.to_string();
    ...
    .execute(&st.db).await?;
    let row = sqlx::query("SELECT * FROM FinanceAccount WHERE id = ?").bind(&id).fetch_one(&st.db).await?;
...
pub async fn get_account(
    ...
    for tr in &txn_rows {
        let cat = fetch_category_mini(&st.db, tr.try_get::<Option<String>, _>("categoryId")?.as_deref()).await?;
        transactions.push(txn_in_account_from_row(tr, cat)?);
    }
    let _ = count;
    Ok(Json(AccountDetail {
        id: row.try_get("id")?, name: row.try_get("name")?, r#type: row.try_get("type")?,
        balance: crate::utils::row_f64(&row, "balance"), currency: row.try_get("currency")?,
        color: row.try_get("color")?, icon: row.try_get("icon")?,
        ...
        count: TransactionCount { transactions: total },
        transactions,
    }))
}
...
pub async fn update_account(
    ...
    let exists: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM FinanceAccount WHERE id = ?")
        .bind(&id).fetch_one(&st.db).await?;
    if exists == 0 { return Err(AppError::NotFound("Account not found".to_string())); }
    let now = now_ms();
    let mut qb: QueryBuilder<Sqlite> = QueryBuilder::new("UPDATE FinanceAccount SET ");
    let mut first = true;
    if let Some(v) = body.get("name").and_then(|v| v.as_str()) { crate::push_set!(qb, first, "name = ", v.to_string()); }
    if let Some(v) = body.get("type").and_then(|v| v.as_str()) { crate::push_set!(qb, first, "type = ", v.to_string()); }
    if let Some(v) = body.get("balance").and_then(|v| v.as_f64()) { crate::push_set!(qb, first, "balance = ", v); }
    if let Some(v) = body.get("currency").and_then(|v| v.as_str()) { crate::push_set!(qb, first, "currency = ", v.to_string()); }
    ...
    if let Some(v) = patch_bool(&body, "isDefault") { crate::push_set!(qb, first, "isDefault = ", if v {1_i64} else {0_i64}); }
    if let Some(v) = patch_bool(&body, "archived") { crate::push_set!(qb, first, "archived = ", if v {1_i64} else {0_i64}); }
...
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct AccountMini { id: String, name: String, r#type: String, color: Option<String> }
...
pub(crate) struct Transaction {
    ...
    recurrence: Option<String>, account_id: String, category_id: Option<String>,
    transfer_to_id: Option<String>, created_at: PrismaDateTime, updated_at: PrismaDateTime,
    account: AccountMini, category: Option<CategoryMini>,
}

#[derive(Serialize)]
pub(crate) struct TxnList { transactions: Vec<Transaction>, total: i64 }

async fn fetch_category_mini(db: &SqlitePool, id: Option<&str>) -> Result<Option<CategoryMini>, sqlx::Error> {
    let id = match id { Some(i) if !i.is_empty() => i, _ => return Ok(None) };
    let row = sqlx::query("SELECT id, name, icon, color, type FROM TransactionCategory WHERE id = ?")
...
fn txn_from_row(r: &sqlx::sqlite::SqliteRow, account: AccountMini, category: Option<CategoryMini>) -> Result<Transaction, sqlx::Error> {
    Ok(Transaction {
        id: r.try_get("id")?, amount: crate::utils::row_f64(r, "amount"),
        description: r.try_get("description")?, r#type: r.try_get("type")?,
        date: PrismaDateTime(r.try_get::<i64, _>("date")?),
        note: r.try_get("note")?,
        is_recurring: r.try_get::<i64, _>("isRecurring")? != 0,
        recurrence: r.try_get("recurrence")?,
        account_id: r.try_get("accountId")?, category_id: r.try_get("categoryId")?,
...
fn txn_in_account_from_row(r: &sqlx::sqlite::SqliteRow, category: Option<CategoryMini>) -> Result<TxnInAccount, sqlx::Error> {
    Ok(TxnInAccount {
        id: r.try_get("id")?, amount: crate::utils::row_f64(r, "amount"),
        description: r.try_get("description")?, r#type: r.try_get("type")?,
        date: PrismaDateTime(r.try_get::<i64, _>("date")?),
...
async fn fetch_account_mini(db: &SqlitePool, id: &str) -> Result<AccountMini, sqlx::Error> {
    ...
    Ok(AccountMini {
        id: r.try_get("id")?, name: r.try_get("name")?,
        r#type: r.try_get("type")?, color: r.try_get("color")?,
    })
}
...
pub async fn list_transactions(
    ...
    let mut cqb: QueryBuilder<Sqlite> = QueryBuilder::new("SELECT COUNT(*) FROM \"Transaction\" WHERE 1=1");
    ...
    for r in &rows {
        ...
        let cat = fetch_category_mini(&st.db, r.try_get::<Option<String>, _>("categoryId")?.as_deref()).await?;
        transactions.push(txn_from_row(r, account, cat)?);
    }
    Ok(Json(TxnList { transactions, total }))
}
...
pub async fn create_transaction(
    ...
    let description = body.get("description").and_then(|v| v.as_str()).map(str::trim).filter(|s| !s.is_empty())
        .ok_or_else(|| AppError::BadRequest("Description is required".to_string()))?.to_string();
    let account_id = body.get("accountId").and_then(|v| v.as_str()).filter(|s| !s.is_empty())
        .ok_or_else(|| AppError::BadRequest("accountId is required".to_string()))?.to_string();
    let date_ms = opt_ms(&body, "date")
        .ok_or_else(|| AppError::BadRequest("Date is required".to_string()))?;
    let txn_type = body.get("type").and_then(|v| v.as_str()).unwrap_or("expense").to_string();
    ...
    let balance_change: f64 = if txn_type == "income" { amount } else if txn_type == "expense" { -amount } else { 0.0 };
    if balance_change != 0.0 {
        sqlx::query("UPDATE FinanceAccount SET balance = balance + ?, updatedAt = ? WHERE id = ?")
            .bind(balance_change).bind(now).bind(&account_id).execute(&st.db).await?;
    }

    let row = sqlx::query("SELECT * FROM \"Transaction\" WHERE id = ?").bind(&id).fetch_one(&st.db).await?;
    let account = fetch_account_mini(&st.db, &account_id).await?;
    let cat = fetch_category_mini(&st.db, row.try_get::<Option<String>, _>("categoryId")?.as_deref()).await?;
...
pub async fn delete_transaction(
    ...
    sqlx::query("DELETE FROM \"Transaction\" WHERE id = ?").bind(&id).execute(&st.db).await?;
...
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct CategoryCount { transactions: i64, budget_items: i64 }

#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct Category {
    id: String, name: String, icon: Option<String>, color: String,
    r#type: String, is_system: bool,
    created_at: PrismaDateTime, updated_at: PrismaDateTime,
    #[serde(rename = "_count")]
    count: CategoryCount,
}
...
pub async fn list_categories(State(st): State<AppState>) -> Result<Json<Vec<Category>>, AppError> {
    ...
    for r in &rows {
        let id: String = r.try_get("id")?;
        let txn_count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM \"Transaction\" WHERE categoryId = ?")
            .bind(&id).fetch_one(&st.db).await?;
        let bi_count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM BudgetItem WHERE categoryId = ?")
            .bind(&id).fetch_one(&st.db).await?;
        out.push(Category {
            id, name: r.try_get("name")?, icon: r.try_get("icon")?,
            color: r.try_get("color")?, r#type: r.try_get("type")?,
            is_system: r.try_get::<i64, _>("isSystem")? != 0,
            created_at: PrismaDateTime(r.try_get::<i64, _>("createdAt")?),
            updated_at: PrismaDateTime(r.try_get::<i64, _>("updatedAt")?),
            count: CategoryCount { transactions: txn_count, budget_items: bi_count },
        });
    }
    Ok(Json(out))
}
...
pub async fn create_category(
    ...
    .bind(now).bind(now)
    .execute(&st.db).await?;
    let row = sqlx::query("SELECT * FROM TransactionCategory WHERE id = ?").bind(&id).fetch_one(&st.db).await?;
    Ok((StatusCode::CREATED, Json(Category {
        id, name: row.try_get("name")?, icon: row.try_get("icon")?,
        color: row.try_get("color")?, r#type: row.try_get("type")?,
        is_system: row.try_get::<i64, _>("isSystem")? != 0,
        created_at: PrismaDateTime(row.try_get::<i64, _>("createdAt")?),
        updated_at: PrismaDateTime(row.try_get::<i64, _>("updatedAt")?),
        count: CategoryCount { transactions: 0, budget_items: 0 },
    })))
}
...
pub(crate) struct BudgetItem {
    id: String, budget_id: String, category_id: String,
    amount: f64, spent: f64,
    created_at: PrismaDateTime, updated_at: PrismaDateTime,
...
async fn budget_items(db: &SqlitePool, budget_id: &str) -> Result<Vec<BudgetItem>, sqlx::Error> {
    let rows = sqlx::query("SELECT * FROM BudgetItem WHERE budgetId = ? ORDER BY createdAt ASC")
        .bind(budget_id).fetch_all(db).await?;
    let mut out = Vec::new();
    for r in &rows {
        let cat_id: String = r.try_get("categoryId")?;
        let cat = fetch_category_mini(db, Some(&cat_id)).await?.unwrap_or(CategoryMini {
            id: cat_id.clone(), name: "Unknown".to_string(), icon: None, color: "#6b7280".to_string(), r#type: "expense".to_string(),
        });
        out.push(BudgetItem {
            id: r.try_get("id")?, budget_id: r.try_get("budgetId")?,
            category_id: cat_id, amount: crate::utils::row_f64(r, "amount"), spent: crate::utils::row_f64(r, "spent"),
            created_at: PrismaDateTime(r.try_get::<i64, _>("createdAt")?),
            updated_at: PrismaDateTime(r.try_get::<i64, _>("updatedAt")?),
            category: cat,
        });
...
pub async fn list_budgets(State(st): State<AppState>) -> Result<Json<Vec<Budget>>, AppError> {
    ...
    for r in &rows {
        ...
        out.push(Budget {
            id, name: r.try_get("name")?, period: r.try_get("period")?,
...
pub async fn create_budget(
    ...
    if let Some(items_arr) = body.get("items").and_then(|v| v.as_array()) {
        for item in items_arr {
            let iid = gen_id();
            let cat_id = item.get("categoryId").and_then(|v| v.as_str()).unwrap_or("").to_string();
    ...
    let row = sqlx::query("SELECT * FROM Budget WHERE id = ?").bind(&id).fetch_one(&st.db).await?;
    let items = budget_items(&st.db, &id).await?;
    Ok((StatusCode::CREATED, Json(Budget {
        id: row.try_get("id")?, name: row.try_get("name")?, period: row.try_get("period")?,
