...
pub async fn list_pomodoro(
    ...
    if let Some(c) = params.get("completed") {
        qb.push(" AND completed = ")
            .push_bind(if c == "true" { 1_i64 } else { 0_i64 });
    }
...
pub async fn create_pomodoro(
    ...
    let duration = body
        ...
        .filter(|&d| d > 0)
        .ok_or_else(|| {
            AppError::BadRequest("Duration is required and must be positive".to_string())
        })?;
    let id = gen_id();
    let now = now_ms();
    ...
    let row = sqlx::query("SELECT * FROM PomodoroSession WHERE id = ?")
        .bind(&id)
        .fetch_one(&st.db)
        .await?;
...
pub async fn update_pomodoro(
    ...
    let exists: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM PomodoroSession WHERE id = ?")
        ...
        .await?;
    if exists == 0 {
        return Err(AppError::NotFound("Pomodoro session not found".to_string()));
    }
    let now = now_ms();
    let mut qb: QueryBuilder<Sqlite> = QueryBuilder::new("UPDATE PomodoroSession SET ");
    let mut first = true;
    if let Some(v) = patch_bool(&body, "completed") {
        crate::push_set!(qb, first, "completed = ", if v { 1_i64 } else { 0_i64 });
    }
    if let Some(v) = patch_ms(&body, "completedAt") {
        crate::push_set!(qb, first, "completedAt = ", v);
    }
    if let Some(v) = body.get("duration").and_then(|v| v.as_i64()) {
        crate::push_set!(qb, first, "duration = ", v);
    }
...
pub async fn delete_pomodoro(
    ...
    sqlx::query("DELETE FROM PomodoroSession WHERE id = ?")
        .bind(&id)
        .execute(&st.db)
        .await?;
