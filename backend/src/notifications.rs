...
pub async fn get_notifications(State(st): State<AppState>) -> Result<Json<serde_json::Value>, AppError> {
    let now = now_ms();
    ...
    for r in &overdue {
        ...
        notifications.push(Notification {
            id: notif_id(), r#type: "overdue-task".to_string(),
            title: "Overdue Task".to_string(),
            description: format!("\"{}\" is {} day{} overdue",
                r.try_get::<String, _>("title").unwrap_or_default(),
                days_over, if days_over > 1 { "s" } else { "" }),
            module: "tasks".to_string(),
            priority: if priority == "urgent" || priority == "high" { "high".to_string() } else { "medium".to_string() },
            created_at: ms_to_iso(due), read: false,
        });
    }
    ...
    if !due_today.is_empty() {
        let names: Vec<String> = due_today.iter().take(2)
            .map(|r| r.try_get::<String, _>("title").unwrap_or_default())
            .collect();
        let remaining = if due_today.len() > 2 { format!(" and {} more", due_today.len() - 2) } else { String::new() };
        notifications.push(Notification {
            id: notif_id(), r#type: "task-due-today".to_string(),
            title: "Tasks Due Today".to_string(),
            description: format!("{} task{} due today: {}{}", due_today.len(),
                if due_today.len() > 1 { "s" } else { "" }, names.join(", "), remaining),
            module: "tasks".to_string(), priority: "medium".to_string(),
    ...
    ).bind(today_ms).fetch_one(&st.db).await?;
    if completed > 0 {
        notifications.push(Notification {
            id: notif_id(), r#type: "task-completed".to_string(),
            title: "Tasks Completed! 🎉".to_string(),
            description: format!("Great job! You completed {} task{} today.", completed, if completed > 1 { "s" } else { "" }),
            module: "tasks".to_string(), priority: "low".to_string(),
            created_at: now_iso.clone(), read: false,
        });
    }
    ...
    if hour >= 10 {
        ...
        for h in &habits {
            ...
            if logged == 0 {
                incomplete += 1;
                if names.len() < 3 { names.push(h.try_get::<String, _>("name").unwrap_or_default()); }
            }
        }
        if incomplete > 0 {
            let remaining = if incomplete > 3 { format!(" and {} more", incomplete - 3) } else { String::new() };
            notifications.push(Notification {
                id: notif_id(), r#type: "habit-reminder".to_string(),
                title: "Habits Pending".to_string(),
                description: format!("{} habit{} not completed today: {}{}", incomplete,
                    if incomplete > 1 { "s" } else { "" }, names.join(", "), remaining),
                module: "habits".to_string(),
                priority: if incomplete >= 3 { "medium".to_string() } else { "low".to_string() },
                created_at: now_iso.clone(), read: false,
            });
        }
    }
    ...
    let all_habits = sqlx::query("SELECT id FROM Habit WHERE archived = 0").fetch_all(&st.db).await?;
    let mut missed_count = 0i64;
    for h in &all_habits {
        let hid: String = h.try_get("id").unwrap_or_default();
        let logged: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM HabitLog WHERE habitId = ? AND date >= ? AND date <= ?"
        ).bind(&hid).bind(yesterday_ms).bind(yesterday_end).fetch_one(&st.db).await?;
        if logged == 0 { missed_count += 1; }
    }
    if missed_count > 0 {
        notifications.push(Notification {
            id: notif_id(), r#type: "habit-missed".to_string(),
            title: "Missed Habits Yesterday".to_string(),
            description: format!("You missed {} habit{} yesterday. Try to get back on track today!", missed_count, if missed_count > 1 { "s" } else { "" }),
            module: "habits".to_string(), priority: "medium".to_string(),
            created_at: ms_to_iso(yesterday_end), read: false,
        });
    }

    // 6. Streak milestones
    let habit_rows = sqlx::query("SELECT id, name FROM Habit WHERE archived = 0").fetch_all(&st.db).await?;
    for h in &habit_rows {
        let hid: String = h.try_get("id").unwrap_or_default();
        ...
        if [7, 14, 30, 60, 90].contains(&streak) {
            let label = match streak { 7 => "One week", 14 => "Two weeks", 30 => "One month", 60 => "Two months", _ => "Three months" };
            notifications.push(Notification {
                id: notif_id(), r#type: "streak-milestone".to_string(),
                title: format!("{} Streak! 🔥", label),
                description: format!("\"{}\" has reached a {}-day streak — keep it going!", hname, streak),
                module: "habits".to_string(),
                priority: if streak >= 30 { "high".to_string() } else if streak >= 14 { "medium".to_string() } else { "low".to_string() },
                created_at: now_iso.clone(), read: false,
            });
        }
    }
    ...
    for g in &approaching_goals {
        ...
        notifications.push(Notification {
            id: notif_id(), r#type: "goal-deadline".to_string(),
            title: "Goal Deadline Approaching".to_string(),
            description: format!("\"{}\" is due in {} day{} ({}% complete)",
                g.try_get::<String, _>("title").unwrap_or_default(), days_left,
                if days_left > 1 { "s" } else { "" }, progress),
            module: "goals".to_string(),
            priority: if days_left <= 2 { "high".to_string() } else if days_left <= 4 { "medium".to_string() } else { "low".to_string() },
            created_at: now_iso.clone(), read: false,
        });
    }
    ...
    for g in &progress_goals {
        let progress: i64 = g.try_get("progress").unwrap_or(0);
        notifications.push(Notification {
            id: notif_id(), r#type: "goal-progress".to_string(),
            title: format!("{}% Progress! 🎯", progress),
            description: format!("\"{}\" is {}% complete. Keep pushing!", g.try_get::<String, _>("title").unwrap_or_default(), progress),
            module: "goals".to_string(),
            priority: if progress >= 75 { "medium".to_string() } else { "low".to_string() },
            created_at: now_iso.clone(), read: false,
        });
    }
    ...
    for g in &completed_goals {
        notifications.push(Notification {
            id: notif_id(), r#type: "goal-completed".to_string(),
            title: "Goal Achieved! 🏆".to_string(),
            description: format!("Congratulations! You completed \"{}\"", g.try_get::<String, _>("title").unwrap_or_default()),
            module: "goals".to_string(), priority: "high".to_string(),
            created_at: now_iso.clone(), read: false,
        });
    }
    ...
    ).bind(now).bind(now).fetch_all(&st.db).await?;
    for bi in &budget_items {
        ...
        if amount > 0.0 && spent / amount > 0.8 {
            ...
            notifications.push(Notification {
                id: notif_id(), r#type: "budget-alert".to_string(),
                title: if over { "Budget Exceeded".to_string() } else { "Budget Warning".to_string() },
                description: format!("{}: ${:.0} of ${:.0} ({}%)", bi.try_get::<String, _>("name").unwrap_or_default(), spent, amount, pct),
                module: "finance".to_string(),
                priority: if over { "high".to_string() } else { "medium".to_string() },
                created_at: now_iso.clone(), read: false,
            });
        }
    }
    ...
    for txn in &large_txns {
        ...
        notifications.push(Notification {
            id: notif_id(), r#type: "large-transaction".to_string(),
            title: "Large Transaction".to_string(),
            description: format!("${:.0} expense: {}", amount, txn.try_get::<String, _>("description").unwrap_or_else(|_| "Unnamed transaction".to_string())),
            module: "finance".to_string(),
            priority: if amount >= 500.0 { "high".to_string() } else { "medium".to_string() },
            created_at: now_iso.clone(), read: false,
        });
    }
    ...
    if hour >= 18 {
        let today_journal: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM JournalEntry WHERE date >= ? AND date <= ?"
        ).bind(today_ms).bind(today_end).fetch_one(&st.db).await?;
        if today_journal == 0 {
            notifications.push(Notification {
                id: notif_id(), r#type: "writing-reminder".to_string(),
                title: "Daily Journal ✍️".to_string(),
                description: "You haven't journalized today. Take a moment to reflect!".to_string(),
                module: "journal".to_string(), priority: "low".to_string(),
                created_at: now_iso.clone(), read: false,
            });
        }
    }
    ...
    let recent_moods = sqlx::query(
        "SELECT mood FROM JournalEntry WHERE mood IS NOT NULL AND date >= ? LIMIT 7"
    ).bind(now - 7 * 86_400_000).fetch_all(&st.db).await?;
    if recent_moods.len() >= 3 {
        ...
        ).bind(now - 7 * 86_400_000).fetch_all(&st.db).await.unwrap_or_default();
        if scores.len() >= 3 {
    ...
    let day_of_week = { let days_since_epoch = (now / 86_400_000) as i64; ((days_since_epoch + 4) % 7) }; // 0=Mon..6=Sun
    if day_of_week == 6 && hour >= 10 {
        notifications.push(Notification {
            id: notif_id(), r#type: "data-backup".to_string(),
            title: "Weekly Backup Reminder".to_string(),
            description: "It's Sunday! Consider backing up your data in Settings → Data.".to_string(),
            module: "settings".to_string(), priority: "low".to_string(),
            created_at: now_iso.clone(), read: false,
        });
    }
    ...
    notifications.sort_by(|a, b| {
        let porder = |p: &str| match p { "high" => 3, "medium" => 2, _ => 1 };
        let pd = porder(&b.priority).cmp(&porder(&a.priority));
        if pd != std::cmp::Ordering::Equal { return pd; }
