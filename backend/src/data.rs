...
pub async fn export_data(State(st): State<AppState>) -> Result<Json<Value>, AppError> {
    ...
    let profile = sqlx::query("SELECT * FROM UserProfile")
        .fetch_all(&st.db)
        .await?;
    ...
    let projects = sqlx::query("SELECT * FROM Project")
        .fetch_all(&st.db)
        .await?;
    let tasks = sqlx::query("SELECT * FROM Task").fetch_all(&st.db).await?;
    let note_folders = sqlx::query("SELECT * FROM NoteFolder")
        .fetch_all(&st.db)
        .await?;
    let notes = sqlx::query("SELECT * FROM Note").fetch_all(&st.db).await?;
    let habits = sqlx::query("SELECT * FROM Habit").fetch_all(&st.db).await?;
    let habit_logs = sqlx::query("SELECT * FROM HabitLog")
        .fetch_all(&st.db)
        .await?;
    let journal_entries = sqlx::query("SELECT * FROM JournalEntry")
        .fetch_all(&st.db)
        .await?;
    let finance_accounts = sqlx::query("SELECT * FROM FinanceAccount")
        .fetch_all(&st.db)
        .await?;
    let transaction_categories = sqlx::query("SELECT * FROM TransactionCategory")
        .fetch_all(&st.db)
        .await?;
    let transactions = sqlx::query("SELECT * FROM \"Transaction\"")
        .fetch_all(&st.db)
        .await?;
    let budgets = sqlx::query("SELECT * FROM Budget")
        .fetch_all(&st.db)
        .await?;
    let budget_items = sqlx::query("SELECT * FROM BudgetItem")
        .fetch_all(&st.db)
        .await?;
    let goals = sqlx::query("SELECT * FROM Goal").fetch_all(&st.db).await?;
    let milestones = sqlx::query("SELECT * FROM Milestone")
        .fetch_all(&st.db)
        .await?;
    let courses = sqlx::query("SELECT * FROM Course")
        .fetch_all(&st.db)
        .await?;
    let course_resources = sqlx::query("SELECT * FROM CourseResource")
        .fetch_all(&st.db)
        .await?;
    let calendar_events = sqlx::query("SELECT * FROM CalendarEvent")
        .fetch_all(&st.db)
        .await?;
    let time_entries = sqlx::query("SELECT * FROM TimeEntry")
        .fetch_all(&st.db)
        .await?;
...
pub async fn import_data(
    ...
    if let Some(arr) = data.get("profile").and_then(|v| v.as_array()) {
        if let Some(p) = arr.first() {
            ...
            if existing > 0 {
                let _ = sqlx::query(
                    "UPDATE UserProfile SET name = ?, email = ?, updatedAt = ? WHERE 1=1",
                )
    ...
    Ok(Json(
        serde_json::json!({ "success": true, "imported": results, "totalImported": total }),
    ))
}
...
pub async fn reset_data(State(st): State<AppState>) -> Result<Json<Value>, AppError> {
    ...
    sqlx::query("DELETE FROM PomodoroSession")
        .execute(&st.db)
        .await?;
    sqlx::query("DELETE FROM CalendarEvent")
        .execute(&st.db)
        .await?;
    sqlx::query("DELETE FROM CourseResource")
        .execute(&st.db)
        .await?;
    sqlx::query("DELETE FROM Course").execute(&st.db).await?;
    sqlx::query("DELETE FROM Milestone").execute(&st.db).await?;
    sqlx::query("DELETE FROM GoalProject")
        .execute(&st.db)
        .await
        .ok();
    sqlx::query("DELETE FROM GoalTag")
        .execute(&st.db)
        .await
        .ok();
    sqlx::query("DELETE FROM Goal").execute(&st.db).await?;
    sqlx::query("DELETE FROM BudgetItem")
        .execute(&st.db)
        .await?;
    ...
    sqlx::query("DELETE FROM \"Transaction\"")
        .execute(&st.db)
        .await?;
    sqlx::query("DELETE FROM TransactionCategory")
        .execute(&st.db)
        .await?;
    sqlx::query("DELETE FROM FinanceAccount")
        .execute(&st.db)
        .await?;
    sqlx::query("DELETE FROM JournalTag")
        .execute(&st.db)
        .await
        .ok();
    sqlx::query("DELETE FROM JournalEntry")
        .execute(&st.db)
        .await?;
    sqlx::query("DELETE FROM HabitLog").execute(&st.db).await?;
    sqlx::query("DELETE FROM HabitTag")
        .execute(&st.db)
        .await
        .ok();
    sqlx::query("DELETE FROM Habit").execute(&st.db).await?;
    sqlx::query("DELETE FROM NoteLink")
        .execute(&st.db)
        .await
        .ok();
    sqlx::query("DELETE FROM NoteBookmark")
        .execute(&st.db)
        .await
        .ok();
    sqlx::query("DELETE FROM NoteTag")
        .execute(&st.db)
        .await
        .ok();
    sqlx::query("DELETE FROM Note").execute(&st.db).await?;
    sqlx::query("DELETE FROM NoteFolder")
        .execute(&st.db)
        .await?;
    sqlx::query("DELETE FROM TaskDependency")
        .execute(&st.db)
        .await
        .ok();
    sqlx::query("DELETE FROM TaskTag")
        .execute(&st.db)
        .await
        .ok();
    ...
    sqlx::query("DELETE FROM UserProfile")
        .execute(&st.db)
        .await?;
    Ok(Json(
        serde_json::json!({ "success": true, "message": "All data has been reset" }),
    ))
}
...
pub async fn get_stats(State(st): State<AppState>) -> Result<Json<Value>, AppError> {
    ...
    let notes: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM Note")
        .fetch_one(&st.db)
        .await?;
    let habits: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM Habit")
        .fetch_one(&st.db)
        .await?;
    let habit_logs: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM HabitLog")
        .fetch_one(&st.db)
        .await?;
    let journal_entries: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM JournalEntry")
        .fetch_one(&st.db)
        .await?;
    let transactions: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM \"Transaction\"")
        .fetch_one(&st.db)
        .await?;
    let goals: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM Goal")
        .fetch_one(&st.db)
        .await?;
    let courses: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM Course")
        .fetch_one(&st.db)
        .await?;
    let calendar_events: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM CalendarEvent")
        .fetch_one(&st.db)
        .await?;
    let time_entries: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM TimeEntry")
        .fetch_one(&st.db)
        .await?;
    ...
    for days_back in 0..365i64 {
        ...
        let has_task: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM Task WHERE createdAt >= ? AND createdAt <= ? LIMIT 1",
        )
        .bind(day_ms)
        .bind(day_end)
        .fetch_one(&st.db)
        .await
        .unwrap_or(0);
        let has_journal: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM JournalEntry WHERE createdAt >= ? AND createdAt <= ? LIMIT 1",
        )
        .bind(day_ms)
        .bind(day_end)
        .fetch_one(&st.db)
        .await
        .unwrap_or(0);
        let has_habit: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM HabitLog WHERE createdAt >= ? AND createdAt <= ? LIMIT 1",
        )
        .bind(day_ms)
        .bind(day_end)
        .fetch_one(&st.db)
    ...
    let earliest: Option<i64> =
        sqlx::query_scalar("SELECT createdAt FROM UserProfile ORDER BY createdAt ASC LIMIT 1")
            .fetch_optional(&st.db)
            .await
            .ok()
            .flatten();
    let account_created = earliest
        .map(|ms| PrismaDateTime(ms).to_iso())
        .unwrap_or_else(|| PrismaDateTime(now).to_iso());
