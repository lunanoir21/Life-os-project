...
pub async fn list_tags(State(st): State<AppState>) -> Result<Json<Vec<Tag>>, AppError> {
    ...
    for r in &rows {
        ...
        let note_tags: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM NoteTag WHERE tagId = ?")
            .bind(&id).fetch_one(&st.db).await?;
        let journal_tags: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM JournalTag WHERE tagId = ?")
            .bind(&id).fetch_one(&st.db).await?;
        ...
        let habit_tags: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM HabitTag WHERE tagId = ?")
            .bind(&id).fetch_one(&st.db).await?;
        let bookmark_tags: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM BookmarkTag WHERE tagId = ?")
            .bind(&id).fetch_one(&st.db).await?;
        ...
        out.push(Tag {
            ...
            count: TagCount { task_tags, note_tags, journal_tags, goal_tags, habit_tags, bookmark_tags },
        });
    }
    Ok(Json(out))
}
